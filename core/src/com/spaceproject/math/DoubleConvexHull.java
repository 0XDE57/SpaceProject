 ...
 * @author Nathan Sweet */
public class DoubleConvexHull {
    private final IntArray quicksortStack = new IntArray();
    private double[] sortedPoints;
    private final DoubleArray hull = new DoubleArray();
    private final IntArray indices = new IntArray();
    private final ShortArray originalIndices = new ShortArray(false, 0);

    /** @see #computePolygon(double[], int, int, boolean) */
    public DoubleArray computePolygon (DoubleArray points, boolean sorted) {
        return computePolygon(points.items, 0, points.size, sorted);
    }

    /** @see #computePolygon(double[], int, int, boolean) */
    public DoubleArray computePolygon (double[] polygon, boolean sorted) {
    ...
    public DoubleArray computePolygon (double[] points, int offset, int count, boolean sorted) {
    ...
    /** @see #computeIndices(double[], int, int, boolean, boolean) */
    public IntArray computeIndices (DoubleArray points, boolean sorted, boolean yDown) {
        return computeIndices(points.items, 0, points.size, sorted, yDown);
    }

    /** @see #computeIndices(double[], int, int, boolean, boolean) */
    public IntArray computeIndices (double[] polygon, boolean sorted, boolean yDown) {
        return computeIndices(polygon, 0, polygon.length, sorted, yDown);
    }

    /** Computes a hull the same as {@link #computePolygon(double[], int, int, boolean)} but returns indices of the specified
     * points. */
    public IntArray computeIndices (double[] points, int offset, int count, boolean sorted, boolean yDown) {
    ...
    private double ccw (double p3x, double p3y) {
        DoubleArray hull = this.hull;
        int size = hull.size;
        double p1x = hull.get(size - 4);
        double p1y = hull.get(size - 3);
        double p2x = hull.get(size - 2);
        double p2y = hull.peek();
    ...
    private int quicksortPartition (final double[] values, int lower, int upper) {
        double x = values[lower];
        double y = values[lower + 1];
        int up = upper;
        int down = lower;
        double temp;
        while (down < up) {
     ...
     * @param count Number of indices, must be even. */
    private void sortWithIndices (double[] values, int count, boolean yDown) {
    ...
    private int quicksortPartitionWithIndices (final double[] values, int lower, int upper, boolean yDown,
